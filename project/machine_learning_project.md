...
* the requirement is  to build a machine learning algorithm to predict activity quality from activity monitors.  
* the goal is to use to use data from accelerometers on the belt, forearm, arm, and dumbell of 6 participants.  
* data provides how these participants performed barbell lifts correctly and incorrectly in 5 different ways. Classe: A, B, C, D, E.  
  
_As part of building a machine learning prediction algorithm_,  
* Chose only classe as a factor variable and variables related to "Accelerometer", x, y and z which are either int or num variables.  
* Peformed EDA to know the type of data (pattern), and it seems to be more like clusters and also a classification problem, thus, we cannot perform linear models on this. Experimented with lvq, k-means and Parallel Random forest but not stated in the project, for the sake of time and space. Other models experimented with lda, qda, glmnet and random forest. Finally, chose random forest.  
* Before we train, we have preprocessed data, so that the models would perform well. Like, remove NAs, zero variables and high corelated variables, whereever need be. Also removed a few rows with outliers. [read more on readme.md]  
* To predict, we have trained the training data using random forest with cross-validated 10 folds, tested using validation data. Several experiments were conducted and finally concluded on random forest. [read more on readme.md] 
* Conducted cross validations on trained data. The different error rates, with different models, are reflected.
* We predict the classe that it belongs to. Predict by passing through the pml-testing data, using the model used to train.    
...
From this plot it is evident that the data is not linear, thus, we cannot process linear models for machine learning.

### Data partition: Training and validation data
...
```
As we have a separate testing data set, splitting this data into only training and validation.
The final testing will be performed on the pmltesting data for prediction.



#### Removal of irrelevant data from training, after EDA
Similar to accel scatter plot, there are scatter plots for gyros and magnet. From Gyros and Magnet scatter plots, it is evident that there are some outliers. For variables, gyros\_dumbbell/forearm x/y/z, magnet\_dumbbell\_y.
...
![](machine_learning_project_files/figure-html/unnamed-chunk-5-1.png) 
...
![](machine_learning_project_files/figure-html/ML_RF_trainData-1.png) 
...
![](machine_learning_project_files/figure-html/ML_tree_trainData-1.png) 
...
From these comparisons it is evident that False rate is on to the higher side.
...
![](machine_learning_project_files/figure-html/unnamed-chunk-12-1.png) 
...
![](machine_learning_project_files/figure-html/unnamed-chunk-13-1.png) 
...
![](machine_learning_project_files/figure-html/unnamed-chunk-16-1.png) 
...
## Sensitivity            0.9845   0.9807   0.9585   0.9944   0.9979
## Specificity            0.9991   0.9929   0.9958   0.9926   0.9991
## Pos Pred Value         0.9978   0.9704   0.9803   0.9619   0.9958
## Neg Pred Value         0.9938   0.9954   0.9910   0.9989   0.9995
## Prevalence             0.2883   0.1914   0.1783   0.1586   0.1834
## Detection Rate         0.2838   0.1877   0.1709   0.1577   0.1830
## Detection Prevalence   0.2845   0.1935   0.1744   0.1639   0.1838
## Balanced Accuracy      0.9918   0.9868   0.9772   0.9935   0.9985
```


```
##                  A       B       C       D       E
## predict    2262.00 1502.00 1399.00 1244.00 1439.00
## validation 2232.00 1518.00 1368.00 1286.00 1442.00
## dif_per       1.33   -1.07    2.22   -3.38   -0.21
## accurcy      98.67   98.93   97.78   96.62   99.79
```
...
![](machine_learning_project_files/figure-html/unnamed-chunk-20-1.png) 
We have got 100% accuracy in predicting the validation data set, in this particular experiment and dataset.

### Predict on testing set and file creation
...
#### One interesting pattern noticed (dumbbell), during EDA.
...
![](machine_learning_project_files/figure-html/unnamed-chunk-22-1.png) 
